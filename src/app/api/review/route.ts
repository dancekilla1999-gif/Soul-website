...
export async function POST(request: Request) {
  ...
  console.info("[SOUL] Новый отзыв:", review);
  ...
  if (reviewsStoreConfigured()) {
    try {
      ...
      console.error("[SOUL] Не удалось сохранить отзыв в очередь:", e);
    }
  } else {
    console.warn("[SOUL] GITHUB_TOKEN не настроен — отзыв не попадёт в /admin");
  }
